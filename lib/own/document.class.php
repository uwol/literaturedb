		...
		*/
		...
		if($count > 0){
			...
			$cmd = sprintf('UPDATE literaturedb_document SET entrytype_id = %s, title = %s, date = %s, abstract = %s, address = %s, booktitle = %s, chapter = %s, doi = %s, ean = %s, edition = %s, institution = %s, journal_id = %s, number = %s, organization = %s, pages = %s, publisher_id = %s, school = %s, series = %s, url = %s, volume = %s, note = %s, rating = %s WHERE id = %s',
				...
				LibDb::secInp(trim($document['abstract'])),
				LibDb::secInp(trim($document['address'])),
				LibDb::secInp(trim($document['booktitle'])),
				LibDb::secInp(trim($document['chapter'])),
				LibDb::secInp(trim($document['doi'])),
				LibDb::secInp(trim($document['ean'])),
				LibDb::secInp(LibDb::zerofy(trim($document['edition']))),
				LibDb::secInp(trim($document['institution'])),
				...
				LibDb::secInp(trim($document['volume'])),
				LibDb::secInp(trim($document['note'])),
